...
public class InstaMineNukerPlus extends Module {
    ...
    public InstaMineNukerPlus() {
        super(VBAddon.CATEGORY, "insta-mine-nuker-plus", "Sends packets to break blocks around you.");
    }
                        ...
                        switch (mode.get()) {
                            case Sphere -> {
                                if (!blocks.contains(blockPos) && distance1 <= reach || distance2 <= reach) {
                                    if (thischunk.get() && !mc.world.getChunk(blockPos).getPos().equals(mc.player.getChunkPos())) break;
                                    blocks.add(blockPos);
                                }
                            }
                            case Box -> {
