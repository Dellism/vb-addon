...
public class NukerPlus extends Module {
    ...
    private final Setting<NukerPlus.Mode> mode = sgGeneral.add(new EnumSetting.Builder<NukerPlus.Mode>()
        .name("mode")
        .description("The way the blocks are broken.")
        .defaultValue(NukerPlus.Mode.Flatten)
        .build()
    );
    ...
    private final Setting<NukerPlus.SortMode> sortMode = sgGeneral.add(new EnumSetting.Builder<NukerPlus.SortMode>()
        .name("sort-mode")
        .description("The blocks you want to mine first.")
        .defaultValue(NukerPlus.SortMode.Closest)
        .build()
    );
    ...
    private final Setting<Boolean> thischunk = sgGeneral.add(new BoolSetting.Builder()
        .name("Only this chunk")
        .description("Don't target blocks outside of the chunk youre in (useful for building slime farms etc). works only in sphere mode")
        .defaultValue(false)
        .build()
    );
    ...
    public NukerPlus() {
        super(VBAddon.CATEGORY, "nuker-plus", "Breaks blocks around you.");
    }
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        BlockIterator.register(Math.max((int) Math.ceil(range.get() + 1), maxh), Math.max((int) Math.ceil(range.get()), maxv), (blockPos, blockState) -> {
            ...
            if (thischunk.get() && !mc.world.getChunk(blockPos).getPos().equals(mc.player.getChunkPos())) return;
